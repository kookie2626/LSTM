...
meters_to_plot = {
    'H1.Z19': 'Tier S (Excellent) Exemplar: H1.Z19',
    'H1.K15': 'Tier A (Good) Exemplar: H1.K15',
    'H1.Z28': 'Tier B (Fair) Exemplar: H1.Z28'
}
...
with psycopg.connect(**CONNECT_KWARGS) as conn:
    for meter_urn, title in meters_to_plot.items():
        sql = f"SELECT ts, value FROM ems.cr_measurement_1h WHERE meter_urn = '{meter_urn}' AND measurement = 'P' ORDER BY ts"
        ...
        plt.title(f'{title} Power Consumption', fontsize=14, fontweight='bold')
        plt.ylabel('Power (W)')
        plt.legend(fontsize=11)
        plt.grid(alpha=0.3)
        plt.tight_layout()
        filename = meter_urn.replace('.', '_')
        plt.savefig(f'{art_dir}/{filename}_usage.png', dpi=120)
        plt.close()

print('Exemplar plots generated successfully.')
